...
class State extends React.Component {
  stateSetter = (initialState) => {
    let state = {};
    if (initialState === null || undefined) {
      return state
    }
  ...
  state = { ...this.stateSetter(this.props.initialState) }
  stateChanger = (value) => {
    this.setState({...this.state , ...value})
  }
  render() {
    return (
      this.props.children({ state: {...this.state}, setState: (value) => {this.stateChanger(value)} })
    )
  }
}
...
const SettingsForm = ({
  ...
  console.log(isNotificationEnabled , isOnlineStatusVisible)
  return (
    <Form>
      <Form.Group>
